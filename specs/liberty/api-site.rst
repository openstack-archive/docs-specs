...
With the Python routes approach, we could first write to the Swagger spec but
then write another lexer for RAML if needed.
...
Russel Sims has done a proof of concept for Volume API v2. He can upload an
...
Generating the Swagger should not require Sphinx as a run-time, ensure that
