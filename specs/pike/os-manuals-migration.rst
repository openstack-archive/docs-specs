...
* ``doc/source/``
  ...
  * ``cli/`` -- anything having to do with using command line tools
    (auto-generated reference info, hand-written tutorials,
    etc.). Because the documentation for each project should live in
    the repository with the code, this directory may not be present
    for all service repositories but will be present for most of the
