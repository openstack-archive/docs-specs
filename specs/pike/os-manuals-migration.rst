...
* ``doc/source/``
  ...
  * ``contributor/`` -- anything related to contributing to the
    ...
    content under /developer, but not all contributors are developers
    and "developer" can be interpreted as "contributor" and "user of
    this library". Service projects should place their automatically
    generated class documentation under this part of the tree, e.g. in
    ``contributor/api`` or ``contributor/internals``.
  ...
  * ``cli/`` -- command line tool reference docs, similar to man pages
    ...
    integration, or manually written). Tutorials or other step-by-step
    guides *using* these tools should go in either the ``user/`` or
    ``admin/`` sections, depending on their audience. Because the
    documentation for each project should live in the repository with
    the code, this directory may not be present for all service
    repositories but will be present for most of the client library
    repositories.
