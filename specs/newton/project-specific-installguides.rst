...
Additionally, it has become apparent through research and outreach that
users, operators, and developers are consistently disappointed by our current
Install Guide, for a number of other, but related, reasons. These tend to
revolve around the document's lack of certain projects, as already mentioned,
and the use of packaged installs, which is one possible way to install
OpenStack - but not the only one.
...
SUSE, and Ubuntu. Also, manual packaged installations provide a single way to
...
and the documentation team acting as enabler for these documents. We also
need to address the audience and use of the existing Install Guide.
